...
*  **Virtualization support for {product-title} on {GCP}.** {product-title} on {GCP} now supports running virtualized workloads using the OpenShift Virtualization operator, leveraging {GCP} C3 bare-metal instances and {GCP} Hyperdisk. This enables you to migrate and modernize virtual machines (VMs) from existing platforms directly onto {product-title}, managing them alongside containerized workloads on a single, unified application platform.
+
For more information about using OpenShift Virtualization on {product-title} on {gcp}, see link:https://docs.redhat.com/en/documentation/openshift_dedicated/4/html/virtualization/index[Virtualization].
