...
*  **Virtualization support for {product-title} on {GCP}.** {product-title} on {GCP} now supports running virtualized workloads using the OpenShift Virtualization operator. This allows you to run both containerized and virtualized workloads on the same cluster, providing flexibility for a wider range of applications and use cases.
+
