...
[source,terminal]
----
$ ocm gcp create dns-zone --domain-prefix=<prefix> --name=<user_defined_name> --project-id=<project_id> --network=<network_id> --network-project-id <network_project_id>
----
+
where:
...
`--name`:: Specifies a user-defined name for the DNS zone. This name is used to identify the DNS zone within your account and must be unique.

`--project-id`:: Specifies the ID of the {GCP} project where the DNS zone will be created.

`--network`:: Specifies the network associated with the DNS zone. This is typically the network where your {product-title} cluster is deployed.
