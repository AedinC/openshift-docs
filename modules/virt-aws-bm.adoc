...
[IMPORTANT]
====
A `Hyperdisk Balanced` storage pool must be created in the {gcp-short} console in the same zone as the C3 metal compute nodes in the {product-title} on {gcp-short} cluster. This storage pool is used for the default storage class in {product-title} on {gcp-short} clusters.
====
endif::openshift-dedicated[]
ifdef::openshift-rosa,openshift-rosa-hcp[]
